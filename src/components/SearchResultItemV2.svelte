...
<button class="flex items-center justify-between gap-3 p-4 w-full text-left transition-colors duration-200 hover:bg-gray-50 {active ? 'bg-blue-50 border-l-2 border-blue-300' : ''}" on:click>
  <div class="flex items-center gap-3 min-w-0">
    ...
    <div class="flex flex-col gap-0.5 min-w-0">
      <span class="text-base text-gray-900 font-medium truncate">{result.kind === "building" ? result.buildingName : result.roomName}</span>
      <span class="text-sm text-gray-500 truncate">{result.kind === "building" ? result.buildingCode : result.roomCode}</span>
    </div>
  </div>
  <span class="inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium bg-blue-50 border border-blue-300 text-blue-600 shrink-0">
    {result.kind === "room" ? "Room" : "Building"}
  </span>
</button>
