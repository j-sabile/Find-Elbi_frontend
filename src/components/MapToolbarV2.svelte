<script lang="ts">
  ...
  // 1. Import the transition from Svelte
  ...
  function handleClick(id: string) {
    if (id === "basemap") showBasemap = !showBasemap;
  }
...
<div class="absolute bottom-4 right-4 z-20 flex flex-col gap-4 items-end">
  {#if showBasemap}
    <div use:clickOutside transition:fly={{ y: 20, duration: 250 }}>
      <BasemapPanelV2 />
    </div>
  {/if}
  ...
  <div class="flex gap-3 bg-white border border-gray-200 rounded-2xl shadow-lg p-4">
    {#each tools as tool}
      <button
        class="flex items-center justify-center w-10 h-10 rounded-xl border border-gray-200 bg-white hover:bg-gray-50 transition-colors duration-200 {tool.id === 'basemap' && showBasemap
