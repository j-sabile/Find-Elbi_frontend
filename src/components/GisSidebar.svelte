  ...
  {:else}
    ...
    <div class="w-80 md:w-96 bg-slate-950/90 border border-slate-800 rounded-xl shadow-2xl backdrop-blur-lg flex flex-col h-full overflow-hidden text-slate-200">
      ...
      <div class="flex bg-slate-900/50 border-b border-slate-800/80 text-[10px] uppercase font-bold tracking-wider">
        <button
          class="flex-1 py-2 text-center border-b-2 hover:bg-slate-900/30 transition-all {activeTab === 'layers' ? 'border-emerald-500 text-emerald-400 bg-slate-950/20' : 'border-transparent text-slate-400'}"
          on:click={() => (activeTab = "layers")}
        >
          Layers
        </button>
        <button
          class="flex-1 py-2 text-center border-b-2 hover:bg-slate-900/30 transition-all {activeTab === 'proximity' ? 'border-emerald-500 text-emerald-400 bg-slate-950/20' : 'border-transparent text-slate-400'}"
          on:click={() => (activeTab = "proximity")}
        >
          Queries
        </button>
        <button
          class="flex-1 py-2 text-center border-b-2 hover:bg-slate-900/30 transition-all {activeTab === 'measure' ? 'border-emerald-500 text-emerald-400 bg-slate-950/20' : 'border-transparent text-slate-400'}"
          on:click={() => (activeTab = "measure")}
        >
          Measure
        </button>
        <button
          class="flex-1 py-2 text-center border-b-2 hover:bg-slate-900/30 transition-all {activeTab === 'attributes' ? 'border-emerald-500 text-emerald-400 bg-slate-950/20' : 'border-transparent text-slate-400'}"
      ...
      <div class="flex-grow overflow-y-auto p-4 space-y-4 text-xs">
          ...
          <div transition:fade={{ duration: 150 }} class="space-y-4">
            <div>
              ...
              <div class="grid grid-cols-2 gap-2">
                <button
                  class="p-2 rounded border text-left flex flex-col gap-1 transition-all {$gisStore.activeBasemap === 'osm' ? 'border-emerald-500 bg-emerald-950/30 text-emerald-200' : 'border-slate-800 bg-slate-900/50 hover:bg-slate-900'}"
                ...
                <button
                  class="p-2 rounded border text-left flex flex-col gap-1 transition-all {$gisStore.activeBasemap === 'carto' ? 'border-emerald-500 bg-emerald-950/30 text-emerald-200' : 'border-slate-800 bg-slate-900/50 hover:bg-slate-900'}"
                  on:click={() => gisStore.setActiveBasemap("carto")}
                >
                  <span class="font-bold text-xs">Light Canvas</span>
                  <span class="text-[9px] text-slate-400">CartoDB Voyager</span>
                </button>
                <button
                  class="p-2 rounded border text-left flex flex-col gap-1 transition-all {$gisStore.activeBasemap === 'satellite' ? 'border-emerald-500 bg-emerald-950/30 text-emerald-200' : 'border-slate-800 bg-slate-900/50 hover:bg-slate-900'}"
                ...
                <button
                  class="p-2 rounded border text-left flex flex-col gap-1 transition-all {$gisStore.activeBasemap === 'topo' ? 'border-emerald-500 bg-emerald-950/30 text-emerald-200' : 'border-slate-800 bg-slate-900/50 hover:bg-slate-900'}"
                  on:click={() => gisStore.setActiveBasemap("topo")}
                >
                  <span class="font-bold text-xs">Topographic</span>
                  <span class="text-[9px] text-slate-400">OpenTopoMap</span>
                </button>
              </div>
            </div>
          ...
          <div transition:fade={{ duration: 150 }} class="space-y-4">
            ...
            <div class="bg-slate-900/50 p-3 rounded-lg border border-slate-800 space-y-3">
              ...
              <div class="space-y-2">
                ...
                <select
                  class="w-full bg-slate-950 border border-slate-800 rounded px-2 py-1 text-slate-200 outline-none focus:border-emerald-500"
                  bind:value={$gisStore.nearestTargetType}
                >
              ...
              <div class="space-y-2">
                ...
                <button
                  class="w-full py-1.5 rounded font-semibold text-center border transition-all {$gisStore.gisTool === 'nearest' ? 'border-amber-500 bg-amber-950/20 text-amber-300' : 'border-slate-800 bg-slate-900 text-slate-300 hover:bg-slate-850'}"
              ...
              {#if $gisStore.nearestResult}
                <div class="p-2.5 rounded bg-amber-950/15 border border-amber-900/30 text-amber-200 mt-2 space-y-1.5" transition:slide>
                  <div class="font-semibold text-xs flex justify-between">
                    ...
                    <button class="text-xs text-amber-400 hover:underline" on:click={() => { if ($gisStore.nearestResult) handleSelectBuilding($gisStore.nearestResult.building); }}>
                      Fly To
                    </button>
                  </div>
              ...
              {:else if $gisStore.gisTool === 'nearest'}
                <div class="text-[10px] text-amber-400/80 italic text-center animate-pulse">
                  Click anywhere on the map to set starting point...
                </div>
              {/if}
            </div>
            ...
            <div class="bg-slate-900/50 p-3 rounded-lg border border-slate-800 space-y-3">
              ...
              <div class="space-y-2">
                <button
                  class="w-full py-1.5 rounded font-semibold text-center border transition-all {$gisStore.gisTool === 'buffer' ? 'border-sky-500 bg-sky-950/20 text-sky-300' : 'border-slate-800 bg-slate-900 text-slate-300 hover:bg-slate-850'}"
              ...
              {:else if $gisStore.gisTool === 'buffer'}
                <div class="text-[10px] text-sky-400/80 italic text-center animate-pulse">
                  Click on the map to run containment query...
                </div>
              {/if}
            </div>
        ...
        {#if activeTab === "measure"}
          <div transition:fade={{ duration: 150 }} class="space-y-4">
            <div class="space-y-2">
              ...
              <div class="grid grid-cols-2 gap-2">
                <button
                  class="py-2 px-3 rounded font-semibold text-xs border flex flex-col items-center justify-center gap-1 transition-all {$gisStore.gisTool === 'measure_dist' ? 'border-emerald-500 bg-emerald-950/20 text-emerald-300' : 'border-slate-800 bg-slate-900 hover:bg-slate-850 text-slate-300'}"
                ...
                <button
                  class="py-2 px-3 rounded font-semibold text-xs border flex flex-col items-center justify-center gap-1 transition-all {$gisStore.gisTool === 'measure_area' ? 'border-emerald-500 bg-emerald-950/20 text-emerald-300' : 'border-slate-800 bg-slate-900 hover:bg-slate-850 text-slate-300'}"
            ...
            {#if $gisStore.gisTool !== 'none' && $gisStore.gisTool.startsWith('measure')}
              <div class="text-[10px] text-emerald-400/80 italic text-center animate-pulse">
                Click multiple locations on the map to plot vertices.
              </div>
            {/if}
          </div>
        {/if}
        ...
        {#if activeTab === "attributes"}
          <div transition:fade={{ duration: 150 }} class="space-y-4">
                ...
                <div class="space-y-1.5">
                  <div class="flex justify-between items-center">
                    ...
                    <button
                      ...
                      on:click={() => copyToClipboard(wkt, "wkt")}
                    >
                      {copyWktStatus}
                    </button>
                  </div>
            ...
            {:else}
              <div class="text-slate-400 text-center py-10 italic">
                Select a building on the map or search using the searchbar to view spatial geometry attributes.
              </div>
            {/if}
          </div>
        {/if}
